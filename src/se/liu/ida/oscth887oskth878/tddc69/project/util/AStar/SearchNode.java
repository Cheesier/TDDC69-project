...
public class SearchNode extends ASearchNode {
    ...
    public ArrayList<ISearchNode> getSuccessors() {
        ...
        if (AStar.level.getTileType(this.x-1, this.y) != null && AStar.level.canPass(this.x-1, this.y, AStar.unit))
            successors.add(new SearchNode(this.x-1, this.y, this, this.goal));

        if (AStar.level.getTileType(this.x+1, this.y) != null && AStar.level.canPass(this.x+1, this.y, AStar.unit))
            successors.add(new SearchNode(this.x+1, this.y, this, this.goal));

        if (AStar.level.getTileType(this.x, this.y+1) != null && AStar.level.canPass(this.x, this.y+1, AStar.unit))
